...
export class ParentalOtpPhonePinGenerateComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<ParentalOtpPhonePinGenerateComponent>, private deviceService: DeviceDetectorService, private _FPS: FingerPrintService,private auth:AuthService,private ds:DataService) {this.timer(1); }
  ...
  resendOtp(){
    ...
    this.auth.ottOtpLogin(formData).subscribe(res => {
   if(res.code == 1){
    this.timerHide = true
    this.timer(1)
    this.clicked = true;
