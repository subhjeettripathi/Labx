...
export class RecentSearchComponent implements OnInit {
  ...
  uid: any;
  show: boolean = false;
  constructor(private ds: DataService, private analyticsService: AnalyticsService, private DEC_SER: DecryptService, private ed: ExchangeDataService, private auth: AuthService, private fcs: FunctionCallingService, private _storage: StorageService, private router: Router, private dialog: MatDialog) {
  ...
  popular_search() {
    ...
    this.ds.recentRes(this.uid).subscribe((data: any) => {
      if (data.code == 1) {
        ...
        if (this.recent_data == "" && this.recent_data != null && this.recent_data.length == 0) {
          this.noSearch = true;
